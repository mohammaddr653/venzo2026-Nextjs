...
const getPriceAndStock = (selectedPropertyvalString: string, product: IProductDocument) => {
  let price = product.price;
  let discount = product.discount;
  let stock = product.stock;

  if (selectedPropertyvalString !== '') {
    const selectiveProperty = product.properties.find((property) => property.selective);
...
export default getPriceAndStock;
